-- ============================================================
-- Year 1 Semester 1 grades — Elon Musk (2026-0004)
-- Section: BSIT-1-1-A | 9 subjects (matches current enlistment; UCO1 11 omitted)
...
-- Remove prior Y1 S1 grade rows for this student (avoids duplicates on re-run; includes UCO1 11 from older scripts)
DELETE g FROM grades g
JOIN courses c ON g.course_id = c.course_id
WHERE g.student_id = @student_id
  AND c.course_code IN (
    'AUS0 11', 'ARPH 11', 'SMMW 11', 'AHU1 11', 'ASS1011',
    'PE1 11', 'ANS1 11', 'AECO 11', 'UPR1 11', 'UCO1 11'
  );

INSERT INTO grades (student_id, course_id, section_id, prelim, midterm, final_grade, semestral_grade, grade, remarks, status, date_recorded, student_name)
...
JOIN courses c ON c.course_code = v.course_code;
-- UCO1 11: part of official Y1 S1 curriculum but not in this student's enlistment — do not seed a grade row.
INSERT INTO grades (student_id, course_id, section_id, prelim, midterm, final_grade, semestral_grade, grade, remarks, status, date_recorded, student_name)
SELECT @student_id, c.course_id,
  COALESCE((SELECT cs.section_id FROM class_sections cs WHERE cs.course_id = c.course_id AND cs.section_code = @section LIMIT 1),
           (SELECT cs.section_id FROM class_sections cs WHERE cs.course_id = c.course_id AND cs.section_code LIKE 'BSIT-1-1%' LIMIT 1),
...
JOIN courses c ON c.course_code = v.course_code;
-- Verify (9 rows — one grade per enlisted Y1 S1 subject)
SELECT c.course_code, g.semestral_grade, g.status
FROM grades g
JOIN courses c ON g.course_id = c.course_id
WHERE g.student_id = @student_id
  AND c.course_code IN ('AUS0 11', 'ARPH 11', 'SMMW 11', 'AHU1 11', 'ASS1011', 'PE1 11', 'ANS1 11', 'AECO 11', 'UPR1 11')
