-- ============================================================
-- Year 4 Semester 2 grades — Elon Musk (2026-0004)
-- Section: BSIT-4-2-A | 2 subjects per SET BSIT official curriculum (Y4 S2)
...
SELECT @student_id, c.course_id,
  COALESCE((SELECT cs.section_id FROM class_sections cs WHERE cs.course_id = c.course_id AND cs.section_code = @section LIMIT 1),
           (SELECT cs.section_id FROM class_sections cs WHERE cs.course_id = c.course_id AND cs.section_code LIKE 'BSIT-4-2%' LIMIT 1),
...
FROM (SELECT 'UCP2 42' AS course_code, 90 AS prelim, 92 AS midterm, 94 AS final_grade, 1.25 AS semestral_grade) v
...
SELECT @student_id, c.course_id,
  COALESCE((SELECT cs.section_id FROM class_sections cs WHERE cs.course_id = c.course_id AND cs.section_code = @section LIMIT 1),
           ...
           (SELECT cs.section_id FROM class_sections cs WHERE cs.course_id = c.course_id LIMIT 1)),
  v.prelim, v.midterm, v.final_grade, v.semestral_grade, v.semestral_grade, 'Passed', 'PASSED', @graded_on, @student_name
FROM (SELECT 'UOJT 42' AS course_code, 88 AS prelim, 90 AS midterm, 92 AS final_grade, 1.5 AS semestral_grade) v
JOIN courses c ON c.course_code = v.course_code;
-- Verify (2 rows expected for this term slice)
SELECT c.course_code, g.semestral_grade, g.status
FROM grades g
JOIN courses c ON g.course_id = c.course_id
WHERE g.student_id = @student_id
  AND c.course_code IN ('UCP2 42', 'UOJT 42')
