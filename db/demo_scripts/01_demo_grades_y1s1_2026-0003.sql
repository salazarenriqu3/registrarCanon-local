-- ============================================================
-- Year 1 Semester 1 grades — Elon Musk (2026-0003)
-- Section: BSIT-1-1-A | 10 subjects per SET BSIT official curriculum (Y1 S1)
...
-- Legacy typo on some DBs (UI shows AFCO 11) — grade the enlisted course if AECO row missing
INSERT IGNORE INTO grades (student_id, course_id, section_id, prelim, midterm, final_grade, semestral_grade, grade, remarks, status, date_recorded, student_name)
SELECT @student_id, c.course_id,
  COALESCE((SELECT cs.section_id FROM class_sections cs WHERE cs.course_id = c.course_id AND cs.section_code = @section LIMIT 1),
           (SELECT cs.section_id FROM class_sections cs WHERE cs.course_id = c.course_id AND cs.section_code LIKE 'BSIT-1-1%' LIMIT 1),
           (SELECT cs.section_id FROM class_sections cs WHERE cs.course_id = c.course_id LIMIT 1)),
  86, 87, 88, 1.75, 1.75, 'Passed', 'PASSED', @graded_on, @student_name
FROM courses c
WHERE c.course_code IN ('AFCO 11', 'AFC0 11')
  AND NOT EXISTS (
      SELECT 1 FROM grades g
      JOIN courses c2 ON g.course_id = c2.course_id
      WHERE g.student_id = @student_id AND c2.course_code = 'AECO 11'
  );
...
WHERE g.student_id = @student_id
  AND c.course_code IN ('AUS0 11', 'ARPH 11', 'SMMW 11', 'AHU1 11', 'ASS1011', 'PE1 11', 'ANS1 11', 'AECO 11', 'AFCO 11', 'AFC0 11', 'UCO1 11', 'UPR1 11')
